...
def get_low_rank(w, rank):
    ...
    if torch.isnan(w).any():
        raise ValueError("Input matrix w contains NaN values.")
    
    u, s, vt = torch.linalg.svd(w, full_matrices=True)

    # Check for NaN values in u, s, vt
    if torch.isnan(u).any() or torch.isnan(s).any() or torch.isnan(vt).any():
        raise ValueError("Inside get low rank SVD produced NaN values in u, s, or vt matrices.")
...
def per_element_weighted_svd(w, weight, rank, layername, max_iter=150, threshold=1e-2):
    """
    :param tensor w: matrix to decompose
    ...
    s = torch.randn(rank, device=s.device)
    prev_norm = (u @ torch.diag(s) @ vt).norm('fro')
    ...
    for i in range(int(max_iter)):
        ...
        cur_norm = (u @ torch.diag(s) @ vt).norm('fro')
        norms.append(cur_norm.item())  # Store L2 norm for plotting
        if torch.abs(prev_norm - cur_norm) <= threshold:
            print(f"exited due to norm condition on iter {i}")
            return u, s, vt
        prev_norm = cur_norm

    # Plotting the L2 norm
    plt.plot(norms)
    plt.xlabel(f"{layername}")
    plt.ylabel("Frobenius Norm")
    plt.title("Convergence of Frobenius Norm")
    # plt.show()
    # plt.savefig(f"plots/convergence_of_l2norm_vs_iterations_per_layer/{layername}_L2_norm.png")  # Save the plot with a specific filename
    plt.savefig(f"plots/convergence_of_frob_norm_vs_iterations_per_layer/{layername}_L2_norm.png") 
...
def decompose_model(model, sensitivity_dict, rank, max_iter, threshold, ratio = None):        
    ...
    if 'lm_head' in layer_names:
        layer_names.remove('lm_head')

    print("layer names after removing full model: ", layer_names)
    ...
    for layername in layer_names:
        ...
        if ratio:
            print(f'ratio compress params: {int((raw_linear.weight.numel()) * ratio)}')
        ...
        print("w, weight shapes : ", w.shape, weight.shape)

        if torch.isnan(w).any():
            raise ValueError("Inside decompose model Input matrix w contains NaN values.")

        if torch.isnan(weight).any():
            raise ValueError("Inside decompose model Input matrix weight contains NaN values.")
        
        u, s, vt = per_element_weighted_svd(w, weight, rank, layername, max_iter, threshold)
        v = vt.T
